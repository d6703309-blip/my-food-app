...
export default async function handler(req) {
  ...
  try {
    ...
    // 🔑 在这里替换成你自己的 Google AI API Key！
    const API_KEY = 'AIzaSyANPBRzRSBquJgA23U5DSIk_4rCPuch--Y'; // ←←← 改成你的真实密钥！
...
饮食模式：${mode || '均衡饮食'}`
    ...
    if (!response.ok) {
      ...
      console.error('Google AI 错误:', err);
    ...
    // 尝试提取纯 JSON（兼容 Gemini 可能返回的 ```json ... ```）
    let result;
    try {
